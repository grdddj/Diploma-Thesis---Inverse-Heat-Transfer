...
class NewCallback:
    ...
    def Call(self, Sim):
        ...
        if Sim.t[-1] > self.last_call + self.Call_at:  # if it is time to comunicate with GUI then show something
            ...
            app.frames[GraphView].refresh_graph_with_new_values(Sim.t[1:],
                    self.TempHistoryAtPoint_x0, self.ExperimentData[0], self.ExperimentData[1])
...
class GraphView(tk.Frame):
    ...
    def __init__(self, parent, controller):
        ...
        self.run_button = tk.Button(self.frame_top, text="Run", bg="green", fg="black",
                                    bd=2, relief="ridge", font=("Calibri", 20),
                                    command=lambda: self.run_button_action())
        self.run_button.place(relx=0.55, rely=0, relheight=1, relwidth=0.15)

        # Button used to pause the simulation
        self.pause_button = tk.Button(self.frame_top, text="Pause", bg="orange", fg="black",
                                    bd=2, relief="ridge", font=("Calibri", 20),
                                    command=lambda: self.pause_button_action())
        self.pause_button.place(relx=0.7, rely=0, relheight=1, relwidth=0.15)

        # Button used to stop the simulation
        self.stop_button = tk.Button(self.frame_top, text="Stop", bg="red", fg="black",
                                    bd=2, relief="ridge", font=("Calibri", 20),
                                    command=lambda: self.stop_button_action())
    ...
    def initialize_settings(self, container):
        ...
        """
        # Rendering the Material Menu on the very top
        self.initialize_material_menu(container)

        # Defined list of all parameters we want to get as an input from the user
        # All the parameters must have the same structure to work properly
        ...
        # Defining characteristics for all the rows and columns
        label_x = 0.00
        label_width = 0.71
        input_x = 0.73
        input_width = 0.27
        row_height = 0.1
        space_between_rows = 0.01

        settings_label_font = ("Calibri", 16)
        settings_entry_font = ("Calibri", 20)

        row_index = 0
        # Rendering all the defined parameters at once in a loop
        for parameter in self.number_parameters_to_get_from_user:
            # Starting from 1, because Material Menu has already taken one row
            row_index += 1
            rely_of_row = row_index * (row_height + space_between_rows)
            label_text = "{} [{}]:".format(parameter["name"], parameter["unit_abbrev"])
            self.label = tk.Label(container, text=label_text, font=settings_label_font)
            self.label.place(relx=label_x, rely=rely_of_row, relheight=row_height, relwidth=label_width)
            setattr(self, parameter["input_name"], tk.Entry(container, font=settings_entry_font))
            getattr(self, parameter["input_name"]).place(relx=input_x, rely=rely_of_row,
                                                        relheight=row_height, relwidth=input_width)
            getattr(self, parameter["input_name"]).insert(0, parameter["default_value"])
    ...
    def save_results_to_png_file(self, figure_which_to_save, initial_temperature,
                                last_time_value, material, method):
        """
        Outputs the (semi)results of a simulation into a PNG file and names it
            accordingly.
        """
        # TODO: discuss the possible filename structure
        file_name = "{}-{}-{}C-{}s.png".format(method, material, int(initial_temperature),
                                            int(last_time_value))
    ...
    def save_results_to_csv_file(self, time_values, temperature_values, material, method):
        ...
        """
        # TODO: discuss the possible filename structure
        file_name = "{}-{}-{}C-{}s.csv".format(method, material, int(temperature_values[0]),
                                            int(time_values[-1]))
    ...
    def pause_button_action(self):
        ...
        if self.simulation_state in ["running", "stopped"]:
            ...
            self.save_results_to_csv_file(self.Sim.t[1:], self.MyCallBack.TempHistoryAtPoint_x0,
                                        self.chosen_material, "NumericalForward")
            self.save_results_to_png_file(self.temperature_figure, self.MyCallBack.TempHistoryAtPoint_x0[0],
                                        self.Sim.t[-1], self.chosen_material, "NumericalForward")
    ...
    def stop_button_action(self):
        ...
        if self.simulation_state in ["running", "paused"]:
            ...
            for parameter in self.number_parameters_to_get_from_user:
                getattr(self, parameter["input_name"])["state"] = "normal"
    ...
    def get_numbers_from_the_user_input(self, list_of_input_elements):
        ...
        for element in list_of_input_elements:
            try:
                ...
                value_from_user = getattr(self, element["input_name"]).get().replace(",", ".")
            ...
            #   and notify the user of it
                ...
                getattr(self, element["input_name"]).delete(0, "end")
                getattr(self, element["input_name"]).insert(0, element["default_value"])
            ...
            getattr(self, element["input_name"])["state"] = "disabled"
    ...
    def PrepareSimulation(self, rho, cp, lmbd):
        ...
        self.Sim = Simulation(Length=self.object_length, Material=my_material, N=self.number_of_elements,
                            HeatFlux=test_q, AmbientTemperature=T_amb, RobinAlpha=13.5)
    ...
    def make_sim_step(self):
        ...
        if self.Sim.t[-1] >= self.t_stop:
            ...
            # Calculating and displaying the error value (average temperature error)
            ErrorNorm = np.sum(abs(self.MyCallBack.TempHistoryAtPoint_x0 - self.T_experiment(self.Sim.t[1:])))/len(self.Sim.t[1:])
        ...
        if self.simulation_state == "running":
            ...
            # Calls itself after 3ms (this might get it laggy if too low) so other
            #   stuff can happen in the app.mainloop in those 3ms
