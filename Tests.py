    ...
    def Call(self, Sim):
        ...
        if Sim.t[-1] > self.last_call + self.Call_at:  # if it is time to comunicate with GUI then show something
            ...
            app.frames[GraphView].refresh_graph_with_new_values(Sim.t[1:],
                    Sim.T_x0, self.ExperimentData[0], self.ExperimentData[1])
...
class GraphView(tk.Frame):
    ...
    def pause_button_action(self):
        ...
        if self.simulation_state in ["running", "stopped"]:
            ...
            self.save_results_to_csv_file(self.Sim.t[1:], self.Sim.T_x0,
                                        self.chosen_material, "NumericalForward")
            self.save_results_to_png_file(self.temperature_figure, self.Sim.T_x0[0],
    ...
    def make_sim_step(self):
        ...
        if self.Sim.t[-1] >= self.t_stop:
            ...
            ErrorNorm = np.sum(abs(self.Sim.T_x0 - self.T_experiment(self.Sim.t[1:])))/len(self.Sim.t[1:])
        ...
        if self.simulation_state == "running":
            ...
            # Calls itself after 3ms (this might get it laggy if too low) so other stuff
            #   can happen in the app.mainloop in those 3ms
